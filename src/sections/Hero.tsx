    ...
    <section id="home" className="relative min-h-screen overflow-hidden pt-28">
      ...
      <div className="section-wrap grid min-h-[calc(100vh-7rem)] items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
        <motion.div initial={{ opacity: 0, y: 38 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
          ...
          <h1 className="font-outfit text-5xl font-semibold leading-[0.98] text-white light:text-ink sm:text-6xl lg:text-7xl uppercase">
        ...
        >
          <div className="absolute inset-8 rounded-full bg-gradient-to-br from-mint/25 via-skyglow/10 to-coral/25 blur-3xl" />
          <div className="glass-panel relative overflow-hidden rounded-[2rem] p-4">
            ...
            <div className="absolute bottom-7 left-7 right-7 z-20 rounded-3xl border border-white/10 bg-night/65 p-5 backdrop-blur-xl light:border-slate-900/10 light:bg-white/85">
              ...
              <p className="mt-2 font-outfit text-xl font-semibold text-white light:text-ink">Junior Frontend Developer at ReactThemes</p>
            </div>
          </div>
      ...
      <a
        href="#about"
        className="absolute bottom-8 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-slate-400 sm:flex"
      >
        Scroll
